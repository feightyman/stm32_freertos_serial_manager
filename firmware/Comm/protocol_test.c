...
{
    static const uint8_t input[] = { 0xAA, 0x02, 0x10, 0x11, 0x22, 0x1E, 0xA9, 0xAA, 0x01, 0x20, 0x33, 0xFB, 0x7A };
    const size_t input_length = sizeof(input) / sizeof(input[0]);
    ProtocolParser_t parser;
    ProtocolFrame_t frame = { 0 };
    ...
    for (index = 7U; index < input_length; index++)
    {
    ...
    Check(frame.data[0] == 0x11U, "frame data[0] matches");
    Check(frame.data[1] == 0x22U, "frame data[1] matches");
    Check(frame.crc_hi == 0x1EU, "crc_hi matches");
    Check(frame.crc_lo == 0xA9U, "crc_lo matches");
    ...
    Check(next_frame.crc_hi == 0xFBU, "crc_hi matches");
    Check(next_frame.crc_lo == 0x7AU, "crc_lo matches");
...
{
    static const uint8_t input[] = { 0x00, 0x55, 0xFF, 0x7E, 0xAA, 0x02, 0x30, 0x11, 0x22, 0x98, 0x6F };
    const size_t input_length = sizeof(input) / sizeof(input[0]);
    ProtocolParser_t parser;
    ProtocolFrame_t frame = {0};
    bool completed = false;
...
static void TestLenZeroFrameWithCRC(void)
{
    static const uint8_t input[] = { 0xAA, 0x00, 0x01, 0x0D, 0x2E };
    const size_t input_length = sizeof(input) / sizeof(input[0]);
    ProtocolParser_t parser;
    ProtocolFrame_t frame = {0};
    bool completed = false;
    ...
    Check(!completed_early, "no frame before final byte");
    Check(completed, "frame completes on final byte");
    Check(frame.len == 0U, "frame length matches");
    Check(frame.cmd == 0x01U, "frame command matches");
    Check(frame.crc_hi == 0x0DU, "crc_hi matches");
    Check(frame.crc_lo == 0x2EU, "crc_lo matches");
    Check(parser.current_state == WAIT_SOF, "parser returns to WAIT_SOF");
    Check(parser.success_count == 0U, "parser data count resets");
}
...
static void TestCRCParsedButNotValidated(void)
{
    ...
    Check(frame.cmd == 0x01U, "frame command matches");
    Check(frame.crc_hi == 0x0DU, "crc_hi matches");
    Check(frame.crc_lo == 0x2FU, "crc_lo matches");
    Check(parser.current_state == WAIT_SOF, "parser returns to WAIT_SOF");
    Check(parser.success_count == 0U, "parser data count resets");
}
...
int main(void)
{
    ...
    TestLenZeroFrameWithCRC();
    TestCRCParsedButNotValidated();
    if (failure_count == 0U)
    {
